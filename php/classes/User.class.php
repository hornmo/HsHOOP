...
<?php // diese Klasse ist die Grundlage für die Zugriffsprüfung; basiert prinzipiell auf einem User-Eintrag in der Datenbank (hier aber nicht implementiert) 
...
class User /* extends DBOObject */ { 
	...
	public function filterList($bodies) { 
		if (is_array($bodies)) { 
			...
			foreach ($bodies as $body) { 
				if (!$this->hasAccess($body)) { 
				// aktuellen Wert aus dem Array entfernen, wenn kein Zugriff besteht 
	...
	private function __construct($userName) {
		...
		diese beiden Methoden füllen das Objekt mit Daten aus der Datenbank $this->userID = $this->findID($this->userName) $this->loadData($this->userID); */
